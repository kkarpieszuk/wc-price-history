...
The free plugin records bundle price history when bundled products use "Priced Individually" (with WC Price History Pro 1.0.1+). **Displaying** the Omnibus lowest price on bundle product pages (bundle total and individually priced bundled items, including variable components with bundle discounts) is a **WC Price History Pro** feature (1.1.0+). Requires the official WooCommerce Product Bundles plugin.
